      ...
      <div class="filterbarpop">
        <div class="tab-bar" v-show="hasTabHeader">
          <a :style="'flex:' + column"
            ...
            v-for="(tab, tabIndex) in menu.tabs" :key="tabIndex"
            :class="selectIndexTab === tabIndex ? 'selected' : ''">
            <span :class="tab.icon"></span>{{tab.name}}
          </a>
        </div>
        <div class="main">
          <div class="main-sidebar" :class="items ? 'bg-style' : 'full-line line-style'">
            <div v-if="menu.type !== 'fitler'">
              <div class="item" @click="clickSidebar(sidemenu, sideMenuIndex)" v-for="(sidemenu, sideMenuIndex) in sideMenus.detailList" :key="sideMenuIndex"
                :class="currentSelectIndex === sideMenuIndex ? 'selected' : ''">
                <span :class="sidemenu.icon"></span>{{ sidemenu.name }}
              </div>
            </div>
            <div v-if="menu.type === 'filter'">
              <div v-for="(sm, _index) in menu.tabs" :key="_index">
                <div class="filter-name">{{sm.name}}</div>
                <div class="filter-item">
                  <span v-for="(sidemenu, index3) in sm.detailList" :key="index3" @click="clickFilterbar(sm, _index, index3)" :class="sidemenu.selectIndex === index3 ? 'item-operation multi-selected' : 'item-operation'">
                  {{ sidemenu.name }}
                  </span>
                </div>
              </div>
          ...
          <div class="main-list line-style" v-if="items">
            <span class="item" @click="clickItem(item, listIndex)" v-for="(item, listIndex) in items.list" :key="listIndex" :class="currentSelectIndex === sideMenus.selectIndex && items.selectIndex === listIndex ? 'selected': ''">{{item.name}}</span>
          </div>
        </div>
      </div>
    </div>
